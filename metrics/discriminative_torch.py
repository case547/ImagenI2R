...
def discriminative_score_metrics(ori_data: np.ndarray, generated_data: np.ndarray, args: argparse.Namespace) -> float:
    ...
    # Network parameters
    hidden_dim = 8 if args.input_size == 1 else int(args.input_size / 2)
    iterations = 2000
    batch_size = 32
    ...
    with torch.no_grad():
        ...
        y_label_final = np.concatenate(
            (
                np.ones(
                    [
                        y_pred_real_curr.shape[1],
                    ]
                ),
                np.zeros(
                    [
                        y_pred_fake_curr.shape[1],
                    ]
                ),
            ),
            axis=0,
